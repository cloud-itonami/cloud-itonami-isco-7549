(ns craftnec.actor
  "CraftNecActor — the ISCO-08 7549 craft workshop scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph` (ADR-2607121000
  / CLAUDE.md Actors section). One graph run = one workshop-coordination
  ...
  resume after human sign-off. ISCO-08 7549 (Craft and Related Workers
  Not Elsewhere Classified) is a broad/generic residual craft-work
  category — varied hand-tools, varied materials, no single dominant
  hazard type — so this actor's scope stays deliberately generic
  ...
  The unconditional invariant: the Craft Trades Coordination Advisor
  can never directly commit an operating record the CraftNecGovernor
  refuses, and can never make this actor finalize a craft-execution
  decision (e.g. deciding a craft item is finished) or a
  product-quality/safety-clearance decision (e.g. declaring an item
  quality- or safety-cleared), nor override a shop safety officer's
  ...
  quality/safety-clearance decisions itself."
...
(defn build-graph
  "Build a compiled CraftNecActor graph. `store` implements
  `craftnec.store/Store`. `advisor` implements
  `craftnec.advisor/Advisor` (defaults to `mock-advisor`).
